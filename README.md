...
A routine started manually enters its first configured phase immediately. Scheduled starts use the routine editor’s weekday checkboxes: after a routine is completed, Routine Beacon queues the earliest upcoming enabled routine on a selected day. At that routine’s first-phase start time, a repeating alarm sounds and Run Mode requires **Dismiss alarm & start routine** before entering the first phase. This makes the scheduled start usable as a wake-up alarm without starting the routine before the person acknowledges it. Routines with no selected days remain manual-only. Each progression makes the upcoming phase current, and the final phase remains visible until the routine is explicitly completed. Completing the last phase automatically queues the next scheduled routine when one is available; the completion screen also offers a prominent **Queue next routine** action, Previous, and Reset Today.
