...
When a routine is started, Run Mode immediately enters the first configured phase and shows its name, instructions, checklist, and scheduled start time; there is no separate ready-to-begin step. Each progression makes the upcoming phase current, and the final phase remains visible until the routine is explicitly completed. From the completion screen, Previous returns to that final phase and the prominent Reset Today button starts the day over.
