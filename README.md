...
When a routine is started, the first configured phase is the upcoming transition. This lets its alarm sound immediately when its scheduled time has already passed instead of silently skipping the first alarm. Advancing begins that phase and shows its name, instructions, checklist, and scheduled start time. Each progression makes the upcoming phase current, and the final phase remains visible until the routine is explicitly completed. From the completion screen, Previous returns to that final phase and the prominent Reset Today button starts the day over.
